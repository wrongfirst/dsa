...
`,n.trim()&&(s+=`#line 1 "test.cpp"
`+n+`
`),!a&&!o&&(s+=`
int main(int argc, char** argv) {
    return 0;
}
`),s}function w(e){if(!e||!e.trim())return[];let t=[],n=/(?:(?:\.\/)?user\.cpp):(\d+):(\d+):\s*(fatal error|error|warning|note):\s*(.*)/g,r,i=null;for(;(r=n.exec(e))!==null;){let e=parseInt(r[1],10)||1,n=parseInt(r[2],10)||1,a=r[3],o=r[4].trim();if(a===`note`){i&&Math.abs((i.line||1)-e)<=1&&(i.message+=` (note: ${o})`);continue}let s={line:e,column:n,endLine:e,endColumn:n+1,message:o,severity:a===`error`||a===`fatal error`?`error`:`warning`,source:`clang++`};t.push(s),i=s}return t}function T(e){if(!e)return``;let t=e.split(`
